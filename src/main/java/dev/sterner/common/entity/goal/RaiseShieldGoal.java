...
public class RaiseShieldGoal extends Goal {
    ...
    protected boolean raiseShield() {
        ...
        if (target != null && guard.shieldCoolDown == 0) {
            ...
            if (guard.distanceTo(target) <= 4.0D || target instanceof CreeperEntity || target instanceof RangedAttackMob && target.distanceTo(guard) >= 5.0D && !ranged || target instanceof RavagerEntity || GuardVillagersConfig.guardAlwaysShield) {
                return true;
            } else {
                return false;
            }
